...
# Normalize
X_normalized, mean_vector, std_vector = normalize_features(X)
...
sample_1 = np.array([1650, 3])
sample_2 = np.array([3000, 4])


def prediction(sample):
    pred = 0
    snorm = (sample - mean_vector) / std_vector
    sn = np.insert(snorm, 0, 1)
    for k in range(theta.shape[0]): #calling calculate_hypothesis function causes error, as sample_id is 1D array and not 2D
        pred = pred + sn[k] * theta_final[k]

    return pred


print(prediction(sample_1))
print(prediction(sample_2))
